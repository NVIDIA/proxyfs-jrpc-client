...
typedef struct bucket_s {
    ...
    TAILQ_HEAD(, bucket_entry_s) entries;
} bucket_t;
...
map_t  *map_init() {
    ...
    map->count = HASH_BUCKET_COUNT;
    map->buckets = (bucket_t **)malloc(sizeof(bucket_t *) * map->count);
    bzero(map->buckets, sizeof(bucket_t *) * map->count);
...
int map_put(map_t *map, p_key_t *key, void *val) {
    ...
    bucket_t *bkt = map->buckets[idx];

    if (bkt == NULL) {
        bkt = (bucket_t *)malloc(sizeof(bucket_t));
        bzero(bkt, sizeof(bucket_t));
        pthread_mutex_init(&bkt->bucket_lock, NULL);
        TAILQ_INIT(&bkt->entries);
        map->buckets[idx] = bkt;
    }

    bucket_entry_t *elm;
    TAILQ_FOREACH(elm, &bkt->entries, entry) {
        if (bcmp(elm->key->ptr, key->ptr, key->ptr_size) == 0) {
    ...
    // Duplicate contents of key passed
    elm->key = make_key(key->ptr, key->ptr_size);
    elm->val = val;
...
static void bucket_free(bucket_t *bkt) {
    while(!TAILQ_EMPTY(&bkt->entries)) {
        bucket_entry_t *elm = TAILQ_FIRST(&bkt->entries);
        TAILQ_REMOVE(&bkt->entries, elm, entry);
        free(elm);
    }
